...
public class Case {
	...
	if (this.piece != null) {
		...
		if(c.getX()>=8 || c.getY()>=8) {
		    System.out.println(this.piece.fenCode());
		}
	...
	for (Coordonnee c : this.menace) {
	    if (plateau[c.getX()][c.getY()].memeCouleur(couleur)
		    && (plateau[c.getX()][c.getY()].getPiece().getClass() != Pion.class || this.getX() != c.getX())) {
		return true;
	    }
	}
	return false;
