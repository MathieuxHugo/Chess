...
public class Fou extends Piece {
	...
	public LinkedList<Coordonnee> coupPossible(int x, int y, Case[][] plateau) {
		...
		while(xTemp<Echiquier.taille && yTemp<Echiquier.taille && plateau[xTemp][yTemp].isVide()) {// Diagonale en bas � droite
		...
		xTemp=x+1;
		yTemp=y-1;
		while(xTemp<Echiquier.taille && yTemp>=0 && plateau[xTemp][yTemp].isVide()) {// Diagonale en haut � droite
			retour.add(new Coordonnee(xTemp,yTemp));
			xTemp++;
			yTemp--;
		}
