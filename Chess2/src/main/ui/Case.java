...
public class Case extends JComponent {
	...
	// getter et setter sur les bool�ens
	...
	public void paintComponent(Graphics g) {
		...
		if (this.choixIcon == null) {
			...
			if (this.subrillance) {
				...
				if (icon != null) {
					for (i = 0; i < 5; i++) {
						g.drawOval(i + this.getWidth() / 20, i + this.getHeight() / 20,
								this.getWidth() - 2 * i - this.getWidth() / 10,
								this.getHeight() - 2 * i - this.getHeight() / 10);
					}
				} else {
					tailleOval = this.getWidth() / 3;
					g.fillOval((this.getWidth() / 2) - tailleOval / 2, (this.getHeight() / 2) - tailleOval / 2,
							tailleOval, tailleOval);
				}
			}
